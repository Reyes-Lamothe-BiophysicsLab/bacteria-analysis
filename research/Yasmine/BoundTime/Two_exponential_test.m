...
Tboundalpha_lb=0.1;%Tboundbetastart;
%Tboundalpha_ub = 5400;%Tboundalphastart*1.7;
Tboundbeta_lb = 0.1;%Tboundbetastart*0.3;
...
%options2 = statset('MaxIter',1000,'MaxFunEvals',1000);

%uMLE=mle(On_time_final, 'pdf',pdf_exp_two_kinetics, 'start',start,'LowerBound',lb,'UpperBound',ub,'options',options2,'OptimFun','fmincon')
...
% uacov = mlecov(uMLE,On_time_final,'pdf',pdf_exp_two_kinetics);
% use = sqrt(diag(uacov))
...
plot(x,pdf_exp_two_kinetics(x,uMLE(1),uMLE(2),uMLE(3),uMLE(4)))
 plot(x,exppdf(x,uMLE(2)));
 plot(x,exppdf(x,uMLE(3)));
...
% cse = sqrt(diag(cacov))
% figure,histogram(data,length(t1),'Normalization','pdf')
% hold on
% plot(x,pdf_exp_two_kinetics(x,cMLE(1),cMLE(2),cMLE(3),cMLE(4)))
% plot(x,exppdf(x,cMLE(2)));
% plot(x,exppdf(x,cMLE(3)));
