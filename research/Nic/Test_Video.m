...
%     num_images_st = length(im2);
    ...
    fn2=strrep(filename_data_tracks{b},'_Trackdata.mat','.tif');
    fn5=append(path5{1,1},fn2);
    im1=bfopen(fn5);
    im2=im1{1, 1};
    num_images_st = length(im2);
    p3=append(path5,p2);
...
%     Pix=regionprops(img2,'Centroid');
    ...
    for i = 1:num_tracks
        ...
        for k = vec_read(1):vec_read(end)
         ...
         ftw.ClickedCallback(); 
        quest_tr = MFquestdlg ( [ 0.6 , 0.1 ] , 'Classification of Molecule', num2str(i),'Yes', 'No', 'Yes');
        switch quest_tr
            case 'No'
                class_input = 0;
            case 'Yes'
