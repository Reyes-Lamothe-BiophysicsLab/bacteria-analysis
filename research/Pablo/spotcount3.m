...
LB = 125; % Pixel Lower Boundary
amp = strel('sphere',4);
LMaxFinder = vision.LocalMaximaFinder('MaximumNumLocalMaxima',4,'NeighborhoodSize',[7,7],'IndexDataType','double','ThresholdSource','Input port');
...
for JJ= 1:height(a)
    ...
    if JJ == 1
        tested = 1;
        while tested == 1
            %user_input_tracks = inputdlg({'Threshold','SNR'},'User Input',[1 50; 1 50],{'75','1.5'});
            %AutoT= (AutoQ/500)*10+150; 
...
legend('Cell Width','Cell Width','One Spot','Two spots','Three Spots', 'Four Spots')
hold off
saveas(gcf,'SpotLocation.pdf')
figure
ints=[{DI(:,5)},{d1(:,5)},{d2(:,5)},{d3(:,5)},{d4(:,5)}];
for l=1:5
    if isempty(ints{1,l})
        ints{1,l} = 0;
    end
