...
dir_tiff = "/Users/nicolassoubry/Analysis/Fiji"
link_dist = 2.0 
gap_link_dist = 2.0 
split_dist = 3.0 
merg_dist = 3.0 
...
for (root, dirs, files) in os.walk(dir_tiff):
    ...
    print(files)
    print (root)
save_analysis_dir = dir_tiff + '/' + 'Analysis_COST' + '_' + 'thr' + str(int_thresh) + '_' + 'lnk' + str(link_dist) + '_' + 'gp' + str(gap_link_dist) + '_' + 'spl' + str(split_dist) + '_' + 'mrg' + str(merg_dist) + '_' + 'costINT' + str(cost_q) + '_GAP' + str(gap) + '_STDspeed_L' + str(STD_SPEED_L)+ '_STDspeed_H' + str(STD_SPEED_H)+ '_Qtrack' + str(qual_spot) + '_NumSpots' + str(num_spots_L) + '_DoG'
...
for z in range(num_files):
    ...
    for id in model.getTrackModel().trackIDs(True):
        ...
        merge = fm.getTrackFeature(id,'NUMBER_MERGES')
        split = fm.getTrackFeature(id,'NUMBER_SPLITS')
        #print(spts)
        ...
        merge_tr.append(merge)
        split_tr.append(split)
        ...
        snr_tot.append(sum(snr_tr)/len(snr_tr))
        x_tr = x_tr + track_x
        y_tr = y_tr + track_y
	...
	qSNR = TMUtils.otsuThreshold(snr_tot)
	
	spt_all_fin_2 = [[row[i] for row in spt_all_fin]
                             for i in range(len(spt_all_fin[0]))]
                             
	vSDTracks = TMUtils.otsuThreshold(std_sp)
         
    data_final = [Track_IDs, spt_tr, spt_widt, mean_sp, max_sp, min_sp, med_sp, std_sp, mean_q, max_q_tr, min_q_tr, med_q_tr, std_q_tr, tr_dur, tr_start, tr_fin, x_lc, y_lc, merge_tr, split_tr ]
    ...
    #set an operation to optimalnumberSpots to change AutoQ

    mqTracks = qTracks*qual_spot
    ...
    filter1 = FeatureFilter('QUALITY', FinalQ, True)
    settings.addSpotFilter(filter1)
    
    #filter3 = FeatureFilter('TOTAL_INTENSITY', 10000, False)
    #settings.addSpotFilter(filter3)

    filter5 = FeatureFilter('SNR', SnR, True)
    ...
    settings.trackerSettings['LINKING_FEATURE_PENALTIES'] = {'QUALITY' : cost_q}
    settings.trackerSettings['GAP_CLOSING_FEATURE_PENALTIES'] = {'QUALITY' : cost_q} 
    #settings.trackerSettings['SPLITTING_FEATURE_PENALTIES'] = {'QUALITY' : cost_q}
    #settings.trackerSettings['MERGING_CLOSING_FEATURE_PENALTIES'] = {'QUALITY' : cost_q} 
    # Configure track analyzers - Later on we want to filter out tracks
    # based on their displacement, so we need to state that we want
    # track displacement to be calculated. By default, out of the GUI,
    # not features are calculated.
    ...
    filter2 = FeatureFilter('TRACK_STD_SPEED', STD_SPEED_L, True)
    settings.addTrackFilter(filter2)

    filter4 = FeatureFilter('NUMBER_SPOTS', num_spots_L , True)
    settings.addTrackFilter(filter4)

    filter8 = FeatureFilter('TRACK_STD_SPEED', STD_SPEED_H, False)
    settings.addTrackFilter(filter8)
    ...
    print("Mean Quality of non filtered Tracks:")
    print(qTracks)
    print("Tracks MQuality:")
    print(mqTracks)
    print("FinalQ Filter")
    print(FinalQ)
    print("Mean StdVelocity before filtering")
    print(vSDTracks)  
