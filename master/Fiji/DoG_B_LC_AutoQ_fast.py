...
for (root, dirs, files) in os.walk(dir_tiff):
    ...
    print(files)
    print (root)
save_analysis_dir = dir_tiff + '/' + 'Analysis_COST' + '_' + 'thr' + str(int_thresh) + '_' + 'lnk' + str(link_dist) + '_' + 'gp' + str(gap_link_dist) + '_' + 'spl' + str(split_dist) + '_' + 'mrg' + str(merg_dist) + '_' + 'costINT' + str(cost_q) + '_GAP' + str(gap) + '_STDspeed' + str(STD_SPEED)+ '_Qtrack' + str(qual_spot) + '_NumSpots' + str(num_spots_L) + '_DoG'
...
for z in range(num_files):
    ...
    for id in model.getTrackModel().trackIDs(True):
        ...
        for spot in track:
            ...
            sstd=spot.getFeature('STD_INTENSITY')
        ...
        sstd_tot.append(sum(sstd_tr)/len(sstd_tr))
	...
	qSTD = TMUtils.otsuThreshold(sstd_tot)
    ...
    filter5 = FeatureFilter('SNR', mqSNR, True)
    settings.addSpotFilter(filter5)
    
    filter7 = FeatureFilter('STD_INTENSITY', qSTD, True)
    settings.addSpotFilter(filter7)
    ...
    filter2 = FeatureFilter('TRACK_STD_SPEED', STD_SPEED, False)
    settings.addTrackFilter(filter2)

    filter4 = FeatureFilter('NUMBER_SPOTS', num_spots_L , True)
    settings.addTrackFilter(filter4)

    filter6 = FeatureFilter('NUMBER_SPOTS', num_spots_H , False)
